...
palabras = open("palabras.txt","r").read().split('\n')
...
def displayBoard(AHORCADO, letraIncorrecta, letraCorrecta, palabraSecreta):
    ...
    for letra in fin: # Mostrará la palabra secreta con espacios entre letras
        print (letra, espacio)
    print ("")
...
while True:
    ...
    if letra in palabraSecreta:
        ...
        if letrasEncontradas:
            print ('¡Muy bien! La palabra secreta es "' + palabraSecreta + '"! ¡Has ganado!'+"\nAbre la última ventana para registrar tu nombre, y regresa para decidir si continuar jugando")
            nombreJugador = simpledialog.askstring("Jugador","Escribe tu nombre:")
            archivoTexto = open("ganadoresAhorcado.txt","a")
            archivoTexto.write("\n"+nombreJugador)
            archivoTexto.close()
            finJuego = True
    else:
    ...
    if finJuego:
        if empezar():
            ...
            print("\nPuuedes regresar al menú principal y probar otros juegos :D\n\n\n\n")
            print("Cierra las ventanas para regresar :D")
            break